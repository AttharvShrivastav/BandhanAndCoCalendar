    ...
    if (joins) {
      for (const [index2, joinMeta] of joins.entries()) {
        ...
        if (is(table2, MySqlTable)) {
          ...
          joinsArray.push(
            sql`${sql.raw(joinMeta.joinType)} join${lateralSql} ${tableSchema ? sql`${sql.identifier(tableSchema)}.` : void 0}${sql.identifier(origTableName)}${useIndexSql2}${forceIndexSql2}${ignoreIndexSql2}${alias2 && sql` ${sql.identifier(alias2)}`} on ${joinMeta.on}`
          );
